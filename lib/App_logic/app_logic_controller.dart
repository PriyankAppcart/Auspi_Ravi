...
import 'package:auspi_ravi_flutter/App_logic/advanced_panchang_model.dart';
import 'package:auspi_ravi_flutter/App_logic/commonStorage.dart';
import 'package:auspi_ravi_flutter/App_logic/planets_extended_model.dart';
import 'package:auspi_ravi_flutter/Moon_yog_calc.dart';
import 'package:auspi_ravi_flutter/Notifications/LocalNotificationService.dart';
import 'package:auspi_ravi_flutter/RemoteServices/RemoteServices.dart';
import 'package:auspi_ravi_flutter/RemoteServices/check_internet.dart';
import 'package:auspi_ravi_flutter/auspi_moon_calc.dart';
import 'package:auspi_ravi_flutter/ghatika_calc.dart';
import 'package:auspi_ravi_flutter/moon_rashi_calc.dart';
import 'package:auspi_ravi_flutter/planets_rashi_nakshatra.dart';
import 'package:auspi_ravi_flutter/sun_calc_data.dart';
import 'package:auspi_ravi_flutter/timezone_select.dart';
...
class app_logic_controller extends GetxController{
  ...
  * */
  ...
  List hora_Monday_list =["Sun-Rajseva","-","-","Moon-good for everything",
    "-","-","Moon-good for everything","-","-"
    ,"-","Sun-Rajseva","-","-","Moon-good for everything",
    "-","-","-","Sun-Rajseva","-",
    "-","Moon-good for everything","-","-","-",
    "Sun-Rajseva"];

  List hora_Tuesday_list =["-","-","-",
    "Mars-Debate/War","Sun-Rajseva","-","Mars-Debate/War","Sun-Rajseva"
    ,"-","-","-","-",
    "-","Mars-Debate/War","Sun-Rajseva","-"
    ,"-","-","-","-",
    "Mars-Debate/War","Sun-Rajseva","-","-","-"];

  List hora_Wednesday_list =["-","Sun-Rajseva","-","Mercury-Learning"
    ,"-","-","Mercury-Learning","-",
    "-","-","-","Sun-Rajseva","-",
    "Mercury-Learning","-","-"
    ,"-","-","Sun-Rajseva","-","Mercury-Learning",
    "-","-","-","-"];

  List hora_Thursday_list =["-","-","-","Jupiter-Marriage"
    ,"-","Sun-Rajseva","Jupiter-Marriage","-","-"
    ,"-","-","-","-","Jupiter-Marriage"
    ,"-","Sun-Rajseva","-","-","-"
    ,"-","Jupiter-Marriage","-","Sun-Rajseva","-","-"];

  List hora_Friday_list =["-","-","Sun-Rajseva","Venus-Journey"
    ,"-","-","Venus-Journey","-","-"
    ,"-","-","-","Sun-Rajseva","Venus-Journey"
    ,"-","-","-","-","-"
    ,"Sun-Rajseva","Venus-Journey","-","-","-","-"];

  List hora_Saturday_list =["-","-","-","Saturn-Saving"
    ,"-","-","Saturn-Saving","-","-"
    ,"Sun-Rajseva","-","-","-","Saturn-Saving"
    ,"-","-","Sun-Rajseva","-","-"
    ,"-","Saturn-Saving","-","-","Sun-Rajseva","-"];
...
if(SunRise!="") {
 ...
 //  print("cdayc$cday");
  ...
  Planetdata=PlanetRashiCalc.getPlanetdata(Venus_deg, SunRise, SunSet, NextSunRise, NextSunSet, PrevSunRise, PrevSunSet, MoonAge, Solar_deg, Hora_map);
  graha = Planetdata[2];
   days = Planetdata[3];
  ...
  Future<dynamic> callApi(isFlag,lat,long) async{
    ...
    try{
      ...
      if(isFlag){
        ...
        Planet_NKSH=PlanetdataDetails[0].nakshatra;
        Planet_Rashi=PlanetdataDetails[0].sign;
        Planet_RashiLord=PlanetdataDetails[0].signLord;
        Planet_NKSH_Lord=PlanetdataDetails[0].nakshatraLord;
        Planet_isRetro=PlanetdataDetails[0].isRetro;
